export const dictionaries = {
  en: {
    // Nav
    "nav.home": "Home",
    "nav.library": "Library",
    "nav.ranking": "Ranking",
    "nav.register": "Register",
    "nav.addNovel": "Add novel",
    "nav.brand": "Shosetu Reader",
    "nav.settings": "Settings",
    "nav.profiles": "Profiles",
    "status.fetched": "Fetched",
    "status.translated": "Translated",
    "status.translationQueued": "Queued",
    "status.translationProcessing": "Translating",
    "status.translationFailed": "Failed",
    "status.fetchPending": "Pending",
    "status.fetchFetching": "Fetching",
    "status.fetchFailed": "Fetch failed",
    "status.liveUpdating": "Live updates are on while background work runs.",
    "status.jobFetchAll": "Fetching episodes",
    "status.jobTranslateAll": "Queueing translations",
    "status.jobWorking": "Working",
    "status.fetchedCount": "{count} fetched",
    "status.queuedCount": "{count} queued",
    "status.failedCount": "{count} failed",
    "status.stop": "Stop",
    "status.confirmStop": "Stop all queued background work for this novel? Currently running steps will finish but no further work will be enqueued.",
    "status.stoppedSummary": "Cancelled — {jobs} job(s), {translations} translation(s).",
    "status.stopFailed": "Could not cancel. Try again.",
    "status.activeBackground": "Background work in progress",
    "status.activeTranslations": "{count} translating",

    // Home
    "home.title": "Shosetu Reader",
    "home.subtitle":
      "A calm reading environment for Japanese web novels from Syosetu, with resume flow and Korean translation.",
    "home.addNovel": "Add novel",
    "home.myLibrary": "My library",
    "home.continueReading": "Continue reading",
    "home.heroVolume": "Vol. {issue} · {year}",
    "home.heroTagline": "Tonight's reading,",
    "home.heroTaglineEm": "quietly",
    "home.heroTaglineSuffix": "translated.",
    "home.heroSubtitle":
      "A cozy environment for reading Japanese web novels from Syosetu — continue, translate to Korean, and discover new stories. All in one place.",
    "home.heroContinueWith": "Continue with",
    "home.thisWeek": "This week's reading",
    "home.statsHours": "Reading time",
    "home.statsEpisodes": "Episodes",
    "home.statsCost": "Translation cost",
    "home.statsHoursValue": "{hours}h",
    "home.statsEpisodesValue": "{count}",
    "home.statsCostValue": "${cost}",
    "home.statsHoursSub": "{streak}-day streak",
    "home.statsEpisodesSub": "in last {days} days",
    "home.statsCostSub": "{models} models used",
    "home.nowReading": "NOW READING",
    "home.bookCount": "{count} books in progress",
    "home.continueAction": "Continue →",
    "home.continueShortcut": "Continue #{episode}",
    "home.viewStats": "→ Detailed stats",

    // Register
    "register.title": "Register a novel",
    "register.subtitle": "Paste a Syosetu URL or enter an ncode to add a novel.",
    "register.placeholder": "https://ncode.syosetu.com/n1234ab/ or n1234ab",
    "register.submit": "Register",
    "register.submitting": "Registering...",
    "register.eyebrow": "Register · multi-source",
    "register.headlineFlair": "Add a",
    "register.headlineMain": "Novel",
    "register.tagline": "Paste any URL — Syosetu · Nocturne · Kakuyomu · AlphaPolis are auto-detected. Bare ncode or composite IDs also accepted.",
    "register.detectedOk": "detected",
    "register.detectedWaiting": "Waiting — type a URL or bare ID to auto-route to one of the four sources.",
    "register.recent": "Recent — guest profile",
    "register.read": "Read",
    "register.networkError": "Network error. Please try again.",
    "register.newlyRegistered": "Newly registered",
    "register.alreadyRegistered": "Already registered",
    "register.by": "by",
    "register.episodes": "episodes",
    "register.completed": "Completed",
    "register.ongoing": "Ongoing",
    "register.viewDetails": "View novel details",

    // Library
    "library.title": "Library",
    "library.subscribedNovel": "subscribed novel",
    "library.subscribedNovels": "subscribed novels",
    "library.empty": "Your library is empty.",
    "library.emptyAction": "Register a novel",
    "library.emptyHint": "and subscribe to start reading.",
    "library.eps": "eps",
    "library.completed": "Completed",
    "library.ongoing": "Ongoing",
    "library.newEpisodes": "New",
    "library.ep": "Ep.",
    "library.heading": "Library",
    "library.headingFlair": "Bibliothèque",
    "library.eyebrow": "My library",
    "library.subscribedSummary": "{count} subscribed novels · {newCount} new episodes unread",
    "library.filterAll": "All",
    "library.filterReading": "Reading",
    "library.filterNew": "New",
    "library.filterCompleted": "Done",
    "library.filterPending": "Queued",
    "library.statusDone": "Translated",
    "library.statusQueued": "Translating",
    "library.statusPending": "Pending",
    "library.statusFailed": "Retry",
    "library.newBadge": "+NEW",
    "library.sync": "Sync",
    "library.continue": "Continue",

    // Novel detail
    "novel.backToLibrary": "Library",
    "novel.completed": "Completed",
    "novel.ongoing": "Ongoing",
    "novel.by": "by",
    "novel.episodes": "episodes",
    "novel.synced": "Synced",
    "novel.viewOnSyosetu": "View on Syosetu",
    "novel.episodesHeading": "Episodes",
    "novel.noEpisodes":
      'No episodes ingested yet. Click "Ingest episodes" above to fetch them from Syosetu.',
    "novel.subscribed": "Subscribed",
    "novel.metaEpisodes": "Episodes",
    "novel.metaTotalChars": "Characters",
    "novel.metaTranslated": "Translated",
    "novel.metaCost": "Translation cost",
    "novel.metaActiveTranslations": "{count} active",
    "novel.metaPending": "{count} pending",
    "novel.metaCharsValue": "{count}",
    "novel.metaUnknown": "—",

    // Subscribe button
    "subscribe.subscribed": "Subscribed",
    "subscribe.subscribe": "Subscribe",

    // Ingest button
    "ingest.actions": "Actions",
    "ingest.ingestGroup": "Ingest",
    "ingest.translateGroup": "Translate",
    "ingest.ingest": "Ingest episodes",
    "ingest.ingesting": "Processing...",
    "ingest.ingestAll": "Ingest all episodes",
    "ingest.ingestAllStarted": "Fetching all episodes in background ({discovered} new discovered)",
    "ingest.result": "Discovered {discovered} new episodes. Fetched {fetched}, failed {failed}.",
    "ingest.networkError": "Network error",
    "ingest.bulkTranslate": "Translate next {count}",
    "ingest.bulkTranslateAll": "Translate all episodes",
    "ingest.bulkTranslating": "Requesting translations...",
    "ingest.bulkTranslateResult": "{queued} translations queued",
    "ingest.bulkTranslateAllStarted": "Translating all {total} episodes in background",
    "ingest.ingestAllProgress": "Fetching episodes {processed}/{total} ({fetched} fetched, {failed} failed)",
    "ingest.bulkTranslateAllProgress": "Queueing translations {processed}/{total} ({queued} queued, {failed} failed)",
    "ingest.backgroundFailed": "Background job failed",
    "ingest.bulkTranslateNone": "No untranslated episodes found",
    "ingest.requestFailed": "Could not start that action. Please try again.",
    "ingest.jobRestored": "Reconnected to the active background job.",
    "ingest.abortTranslation": "Abort translation",
    "ingest.abortSuccess": "Translation session aborted",
    "ingest.abortFailed": "Failed to abort",
    "ingest.noActiveSession": "No active translation session",
    "ingest.reingestAll": "Re-ingest all episodes",
    "ingest.reingestAllStarted": "Re-fetching {reset} episodes in background",
    "ingest.reingestNone": "No episodes to re-ingest",
    "ingest.reingestConfirm": "This will re-fetch all episodes from the source. Continue?",
    "ingest.bulkRetranslate": "Re-translate all",
    "ingest.bulkRetranslateStarted": "Re-translating {total} episodes with {model}",
    "ingest.bulkRetranslateNone": "No translated episodes to re-translate",

    // Reader
    "reader.episodeList": "Episode list",
    "reader.previous": "Previous",
    "reader.next": "Next",
    "reader.noContent": "Episode content has not been fetched yet.",
    "reader.glossaryHeading": "Glossary",
    "reader.glossaryEmpty": "No confirmed glossary entries yet.",
    "reader.glossaryShow": "Show glossary",
    "reader.glossaryHide": "Hide glossary",
    "reader.styleGuideHeading": "Style guide",
    "reader.episodeOf": "{number} of {total}",
    "reader.endOfNovel": "End ✓",

    // Translation toggle
    "translation.translate": "Translate",
    "translation.translating": "Translating...",
    "translation.bilingual": "Show Korean and Japanese paragraphs side by side",
    "translation.failedRetry": "Failed — retry",
    "translation.rateLimited": "Rate limited — try another model",
    "translation.retranslate": "Re-translate",
    "translation.selectModel": "Select translation",
    "translation.model": "Model",
    "translation.currentModel": "Current model",
    "translation.openSettings": "Open model settings",
    "translation.noSavedTranslations": "No saved translations for this episode yet.",
    "translation.inventoryTitle": "Translation inventory",
    "translation.inventorySubtitle": "Remove saved translations before re-running with a new prompt or model.",
    "translation.availableCount": "Available",
    "translation.discardAll": "Discard all",
    "translation.discardModel": "Discard model",
    "translation.discardSingle": "Discard translation",
    "translation.discardShort": "Del",
    "translation.discarding": "Discarding...",
    "translation.inventoryEmpty": "No saved translations for this novel yet.",
    "translation.confirmDiscardSingle": "Discard this saved translation?",
    "translation.confirmDiscardAll": "Discard all saved Korean translations for this novel?",
    "translation.confirmDiscardModel": "Discard saved translations for model {model}?",
    "translation.totalCost": "Total cost",
    "translation.costLabel": "Cost",
    "translation.costValue": "${cost}",
    "translation.costNotAvailable": "Cost N/A",
    "translation.modelCount": "{count} models",
    "translation.requestFailed": "Could not request translation. Please try again.",
    "translation.loadFailed": "Could not load that translation.",
    "translation.discardFailed": "Could not discard the saved translation.",
    "translation.discardSuccess": "Saved translation discarded.",
    "translation.etaLabel": "Est. remaining",
    "translation.confidenceLow": "Low confidence",
    "translation.confidenceMedium": "Medium confidence",
    "translation.confidenceHigh": "High confidence",

    // Ranking
    "ranking.title": "Ranking",
    "ranking.subtitle": "Discover popular novels on Syosetu.",
    "ranking.daily": "Daily",
    "ranking.weekly": "Weekly",
    "ranking.monthly": "Monthly",
    "ranking.quarterly": "Quarterly",
    "ranking.loading": "Loading rankings...",
    "ranking.empty": "No ranking data available.",
    "ranking.eps": "eps",
    "ranking.completed": "Completed",
    "ranking.ongoing": "Ongoing",
    "ranking.view": "View",
    "ranking.register": "Register",
    "ranking.translatingTitles": "Translating titles...",
    "ranking.eyebrow": "Discover",
    "ranking.headline": "Top of Syosetu",
    "ranking.headlineFlair": "Top of",
    "ranking.headlineMain": "Syosetu",
    "ranking.tagline": "Daily Syosetu rankings — refreshed every day, titles translated to Korean.",

    // Settings
    "settings.title": "Settings",
    "settings.language": "Language",
    "settings.readerSettings": "Reader settings",
    "settings.readerSettingsDesc": "Font size · line height · content width — applies to the reader screen.",
    "settings.fontFamily": "Font",
    "settings.fontFamilyDesc": "Body typography for source kanji and kana. Click a preview to apply.",
    "settings.fontWeight": "Weight",
    "settings.fontSize": "Size",
    "settings.lineHeight": "Line",
    "settings.contentWidth": "Width",
    "settings.save": "Save",
    "settings.saving": "Saving...",
    "settings.saved": "Saved",
    "settings.toggleTheme": "Toggle theme",
    "settings.themePaper": "Paper",
    "settings.themeSepia": "Sepia",
    "settings.themeNight": "Night",
    "settings.themeSystem": "System",
    "settings.themeSection": "Theme",
    "settings.themeSectionDesc": "Switch between paper, sepia, night, or system mode.",
    "settings.adultContent": "Adult sources",
    "settings.adultContentDesc": "Show R-18 sources (Nocturne, AlphaPolis adult-flagged) in ranking, search, and library.",
    "settings.adultContentOn": "Visible",
    "settings.adultContentOff": "Hidden",
    "source.syosetu": "Syosetu",
    "source.nocturne": "Nocturne",
    "source.kakuyomu": "Kakuyomu",
    "source.alphapolis": "AlphaPolis",
    "ranking.period.daily": "Daily",
    "ranking.period.weekly": "Weekly",
    "ranking.period.monthly": "Monthly",
    "ranking.period.quarterly": "Quarterly",
    "ranking.period.yearly": "Yearly",
    "ranking.period.entire": "All-time",
    "ranking.period.hot": "Hot",
    "ranking.scope.sfw": "Safe-only",
    "ranking.scope.all": "All sources",
    "ranking.sectionTimeout": "Source slow — retry?",
    "ranking.sectionError": "Source unavailable",
    "settings.translationConfig": "Configure translation model and prompts.",
    "settings.translationModel": "Translation model",
    "settings.translationModelDesc": "Select the OpenRouter model used for JA→KO translation. Workload-specific picks below override this default.",
    "settings.currentModel": "Current model",
    "settings.workloadModels": "Workload models",
    "settings.workloadModelsDesc": "Override the default model per workload — e.g. Flash for routine translation, Pro for glossary bootstrap. Leaving a slot empty inherits the env default or the global model above.",
    "settings.workloadTranslate": "Translate · body",
    "settings.workloadTranslateHint": "Episode body translation. Reasoning OFF, output cost dominates.",
    "settings.workloadTitle": "Title batch",
    "settings.workloadTitleHint": "Ranking and episode title batches. Short outputs.",
    "settings.workloadSummary": "Session summary",
    "settings.workloadSummaryHint": "Rolling context summaries between episodes.",
    "settings.workloadExtraction": "Glossary extract",
    "settings.workloadExtractionHint": "Per-episode JSON extraction of new terms.",
    "settings.workloadCompare": "Compare / retranslate",
    "settings.workloadCompareHint": "Quality-sensitive single-shot retranslations.",
    "settings.workloadBootstrap": "Glossary bootstrap",
    "settings.workloadBootstrapHint": "Cold-start glossary mining from JA-only morph candidates.",
    "settings.workloadInherit": "inherits",
    "settings.workloadInheritPlaceholder": "Inherit (leave empty)",
    "settings.workloadReset": "Reset to inherit",
    "settings.searchModels": "Search models...",
    "settings.loadingModels": "Loading available models...",
    "settings.noModelsFound": "No models found.",
    "settings.refineSearch": "Showing first 50 results. Refine your search for more.",
    "settings.globalPrompt": "Global translation prompt",
    "settings.globalPromptDesc": "Additional instructions applied to all translations.",
    "settings.globalPromptPlaceholder": "Enter additional translation guidelines (e.g. honorific handling, style preferences)...",
    "settings.useDefault": "Use default",
    "settings.eyebrow": "Preferences",
    "settings.heading": "Settings",
    "settings.headingFlair": "Settings",
    "settings.sectionAccount": "Account · Profiles",
    "settings.sectionReading": "Reading",
    "settings.sectionTranslation": "Translation",
    "settings.sectionData": "Data · Sync",
    "settings.profilesHint": "Manage local reader profiles and switch between personalized settings.",
    "settings.profilesAction": "Open profiles →",
    "settings.themeSepiaBg": "#f1e2c7",

    // Profiles
    "profile.loading": "Loading profiles...",
    "profile.title": "Profiles",
    "profile.subtitle": "Create local reader profiles and switch between personalized settings and reading progress.",
    "profile.create": "Create profile",
    "profile.creating": "Creating...",
    "profile.namePlaceholder": "Profile name",
    "profile.importGuestData": "Import current guest data into this profile",
    "profile.listTitle": "Available profiles",
    "profile.select": "Select",
    "profile.switching": "Switching...",
    "profile.current": "Current",
    "profile.empty": "No profiles yet.",
    "profile.guest": "Guest profile",
    "profile.useGuest": "Use guest",
    "profile.manage": "Manage",
    "profile.active": "Active",
    "profile.loadFailed": "Could not load profiles. Refresh and try again.",
    "profile.createFailed": "Could not create the profile. Please check the name and try again.",
    "profile.selectFailed": "Could not switch profiles. Please try again.",
    "profile.useGuestFailed": "Could not switch to guest mode. Please try again.",
    "profile.createSuccess": "Profile created and activated.",
    "profile.switchSuccess": "Profile switched.",
    "profile.guestSuccess": "Guest profile is active.",

    // Glossary
    "glossary.title": "Glossary & guidelines",
    "glossary.subtitle": "Generate entries & style guide from translated episodes",
    "glossary.placeholder": "Prose style guidelines (tone, register, speech patterns). Term mappings are managed in the entries table above.",
    "glossary.generate": "Generate",
    "glossary.generating": "Generating...",
    "glossary.generatedInfo": "Generated from {episodes} episodes on {date}",
    "glossary.generateAction": "Generate entries & style guide",
    "glossary.generateActionBusy": "Generating...",
    "glossary.model": "Model",
    "glossary.estimatedCost": "Est. cost",
    "glossary.noEpisodes": "No translated episodes available",
    "glossary.episodes": "{count} episodes",
    "glossary.entries": "Glossary entries",
    "glossary.styleGuide": "Style guide",
    "glossary.addEntry": "Add",
    "glossary.termJa": "Japanese",
    "glossary.termKo": "Korean",
    "glossary.reading": "Reading",
    "glossary.notes": "Notes",
    "glossary.category": "Category",
    "glossary.statusConfirmed": "Confirmed",
    "glossary.statusSuggested": "Suggested",
    "glossary.statusRejected": "Rejected",
    "glossary.confirm": "Confirm",
    "glossary.reject": "Reject",
    "glossary.delete": "Delete",
    "glossary.all": "All",
    "glossary.character": "Character",
    "glossary.place": "Place",
    "glossary.term": "Term",
    "glossary.skill": "Skill",
    "glossary.honorific": "Honorific",
    "glossary.noEntries": "No glossary entries yet.",
    "glossary.entryAdded": "Entry added",
    "glossary.entryUpdated": "Entry updated",
    "glossary.entryDeleted": "Entry deleted",
    "glossary.entryAddFailed": "Failed to add entry",
    "glossary.entryUpdateFailed": "Failed to update entry",
    "glossary.entryDeleteFailed": "Failed to delete entry",
    "glossary.saveFailed": "Failed to save style guide",
    "glossary.generateFailed": "Generation failed",
    "glossary.networkError": "Network error",
    "glossary.generated": "Style guide generated",
    "glossary.generatedWithEntries": "Style guide generated, {count} entries imported",
    "glossary.loading": "Loading...",
    "glossary.save": "Save",
    "glossary.cancel": "Cancel",
    "glossary.importance": "Imp",

    // Auth
    "auth.loading": "Loading...",
    "auth.title": "Sign in",
    "auth.subtitle": "Use an email to load your personal library, settings, and continue-reading state.",
    "auth.email": "Email",
    "auth.displayName": "Display name",
    "auth.displayNamePlaceholder": "Optional name",
    "auth.signIn": "Sign in",
    "auth.signingIn": "Signing in...",
    "auth.signOut": "Sign out",
    "auth.signedInAs": "Signed in as",
    "auth.invalidCredentials": "Please check your details and try again.",
    "auth.networkError": "Network error. Please try again.",
    "auth.guestHint": "Prefer not to sign in?",
    "auth.continueAsGuest": "Continue as guest",

    // Locale names
    "locale.en": "English",
    "locale.ko": "한국어",
  },

  ko: {
    // Nav
    "nav.home": "홈",
    "nav.library": "서재",
    "nav.ranking": "랭킹",
    "nav.register": "등록",
    "nav.addNovel": "소설 추가",
    "nav.brand": "나로우 리더",
    "nav.settings": "설정",
    "nav.profiles": "프로필",
    "status.fetched": "수집 완료",
    "status.translated": "번역 완료",
    "status.translationQueued": "대기 중",
    "status.translationProcessing": "번역 중",
    "status.translationFailed": "실패",
    "status.fetchPending": "대기 중",
    "status.fetchFetching": "수집 중",
    "status.fetchFailed": "수집 실패",
    "status.liveUpdating": "백그라운드 작업이 진행되는 동안 자동으로 갱신됩니다.",
    "status.jobFetchAll": "에피소드 수집 중",
    "status.jobTranslateAll": "번역 대기열 추가 중",
    "status.jobWorking": "작업 중",
    "status.fetchedCount": "{count}화 수집",
    "status.queuedCount": "{count}화 대기열 추가",
    "status.failedCount": "{count}화 실패",
    "status.stop": "중지",
    "status.confirmStop": "이 소설의 모든 대기열 작업을 중지할까요? 진행 중인 단계는 완료되지만 새 작업은 추가되지 않습니다.",
    "status.stoppedSummary": "중지됨 — 작업 {jobs}건, 번역 {translations}건 취소.",
    "status.stopFailed": "중지에 실패했습니다. 다시 시도해 주세요.",
    "status.activeBackground": "백그라운드 작업 진행 중",
    "status.activeTranslations": "{count}화 번역 중",

    // Home
    "home.title": "나로우 리더",
    "home.subtitle":
      "나로우 일본 웹소설을 위한 편안한 읽기 환경. 이어읽기와 한국어 번역 지원.",
    "home.addNovel": "소설 추가",
    "home.myLibrary": "내 서재",
    "home.continueReading": "이어서 읽기",
    "home.heroVolume": "Vol. {issue} · {year}년",
    "home.heroTagline": "오늘의 독서,",
    "home.heroTaglineEm": "조용히",
    "home.heroTaglineSuffix": "번역됩니다.",
    "home.heroSubtitle":
      "나로우 일본 웹소설을 위한 편안한 읽기 환경. 이어읽기, 한국어 번역, 개인 서재, 작품 발견까지 — 한 곳에서.",
    "home.heroContinueWith": "이어 읽기",
    "home.thisWeek": "이번 주 읽기",
    "home.statsHours": "읽은 시간",
    "home.statsEpisodes": "에피소드",
    "home.statsCost": "번역 비용",
    "home.statsHoursValue": "{hours}시간",
    "home.statsEpisodesValue": "{count}",
    "home.statsCostValue": "${cost}",
    "home.statsHoursSub": "{streak}일 연속",
    "home.statsEpisodesSub": "최근 {days}일",
    "home.statsCostSub": "{models}개 모델 사용",
    "home.nowReading": "지금 읽는 중",
    "home.bookCount": "{count}권 진행 중",
    "home.continueAction": "계속 읽기 →",
    "home.continueShortcut": "이어서 #{episode}",
    "home.viewStats": "→ 자세한 통계",

    // Register
    "register.title": "소설 등록",
    "register.subtitle": "나로우 URL을 붙여넣거나 ncode를 입력하세요.",
    "register.placeholder": "https://ncode.syosetu.com/n1234ab/ 또는 n1234ab",
    "register.submit": "등록",
    "register.submitting": "등록 중...",
    "register.networkError": "네트워크 오류. 다시 시도해 주세요.",
    "register.eyebrow": "등록 · 다중 소스",
    "register.headlineFlair": "Add a",
    "register.headlineMain": "Novel",
    "register.tagline": "URL을 붙여넣기만 하세요. Syosetu · Nocturne · Kakuyomu · AlphaPolis 모두 자동 인식됩니다. ncode / 합성 ID 직접 입력도 가능합니다.",
    "register.detectedOk": "인식됨",
    "register.detectedWaiting": "대기 중 — 입력하면 4개 사이트 중 하나로 자동 라우팅됩니다.",
    "register.recent": "최근 등록 — 게스트 프로필",
    "register.read": "읽기",
    "register.newlyRegistered": "새로 등록됨",
    "register.alreadyRegistered": "이미 등록됨",
    "register.by": "작가",
    "register.episodes": "화",
    "register.completed": "완결",
    "register.ongoing": "연재 중",
    "register.viewDetails": "소설 상세 보기",

    // Library
    "library.title": "서재",
    "library.subscribedNovel": "구독 소설",
    "library.subscribedNovels": "구독 소설",
    "library.empty": "서재가 비어있습니다.",
    "library.emptyAction": "소설을 등록",
    "library.emptyHint": "하고 구독하여 읽기를 시작하세요.",
    "library.eps": "화",
    "library.completed": "완결",
    "library.ongoing": "연재 중",
    "library.newEpisodes": "새 에피소드",
    "library.ep": "화",
    "library.heading": "서재",
    "library.headingFlair": "Bibliothèque",
    "library.eyebrow": "나의 서재",
    "library.subscribedSummary": "구독 중인 작품 {count}편 · 미열람 신규 에피소드 {newCount}편",
    "library.filterAll": "전체",
    "library.filterReading": "읽는 중",
    "library.filterNew": "신규",
    "library.filterCompleted": "완독",
    "library.filterPending": "대기",
    "library.statusDone": "번역 완료",
    "library.statusQueued": "번역 중",
    "library.statusPending": "번역 대기",
    "library.statusFailed": "재시도",
    "library.newBadge": "+NEW",
    "library.sync": "동기화",
    "library.continue": "이어 읽기",

    // Novel detail
    "novel.backToLibrary": "서재",
    "novel.completed": "완결",
    "novel.ongoing": "연재 중",
    "novel.by": "작가",
    "novel.episodes": "화",
    "novel.synced": "동기화",
    "novel.viewOnSyosetu": "나로우 사이트에서 보기",
    "novel.episodesHeading": "에피소드",
    "novel.noEpisodes":
      '에피소드가 아직 수집되지 않았습니다. 위의 "에피소드 수집" 버튼을 클릭하여 나로우에서 가져오세요.',
    "novel.subscribed": "구독 중",
    "novel.metaEpisodes": "에피소드",
    "novel.metaTotalChars": "글자 수",
    "novel.metaTranslated": "번역됨",
    "novel.metaCost": "번역 비용",
    "novel.metaActiveTranslations": "{count}화 진행",
    "novel.metaPending": "{count}화 대기",
    "novel.metaCharsValue": "{count}",
    "novel.metaUnknown": "—",

    // Subscribe button
    "subscribe.subscribed": "구독 중",
    "subscribe.subscribe": "구독",

    // Ingest button
    "ingest.actions": "작업",
    "ingest.ingestGroup": "수집",
    "ingest.translateGroup": "번역",
    "ingest.ingest": "에피소드 수집",
    "ingest.ingesting": "처리 중...",
    "ingest.ingestAll": "전체 에피소드 수집",
    "ingest.ingestAllStarted": "백그라운드에서 전체 에피소드 수집 중 (신규 {discovered}화 발견)",
    "ingest.result": "새 에피소드 {discovered}화 발견. {fetched}화 수집, {failed}화 실패.",
    "ingest.networkError": "네트워크 오류",
    "ingest.bulkTranslate": "다음 {count}화 번역",
    "ingest.bulkTranslateAll": "전체 에피소드 번역",
    "ingest.bulkTranslating": "번역 요청 중...",
    "ingest.bulkTranslateResult": "{queued}화 번역 대기열 추가",
    "ingest.bulkTranslateAllStarted": "전체 {total}화 백그라운드 번역 시작",
    "ingest.ingestAllProgress": "에피소드 수집 중 {processed}/{total} ({fetched}화 완료, {failed}화 실패)",
    "ingest.bulkTranslateAllProgress": "번역 대기열 추가 중 {processed}/{total} ({queued}화 대기열, {failed}화 실패)",
    "ingest.backgroundFailed": "백그라운드 작업이 실패했습니다",
    "ingest.bulkTranslateNone": "미번역 에피소드가 없습니다",
    "ingest.requestFailed": "작업을 시작하지 못했습니다. 다시 시도해 주세요.",
    "ingest.jobRestored": "진행 중인 백그라운드 작업에 다시 연결했습니다.",
    "ingest.abortTranslation": "번역 중단",
    "ingest.abortSuccess": "번역 세션이 중단되었습니다",
    "ingest.abortFailed": "중단 실패",
    "ingest.noActiveSession": "진행 중인 번역 세션이 없습니다",
    "ingest.reingestAll": "전체 에피소드 재수집",
    "ingest.reingestAllStarted": "{reset}화 백그라운드에서 재수집 시작",
    "ingest.reingestNone": "재수집할 에피소드가 없습니다",
    "ingest.reingestConfirm": "모든 에피소드를 원본에서 다시 가져옵니다. 계속하시겠습니까?",
    "ingest.bulkRetranslate": "전체 재번역",
    "ingest.bulkRetranslateStarted": "{total}화 {model}로 재번역 중",
    "ingest.bulkRetranslateNone": "재번역할 번역된 에피소드가 없습니다",

    // Reader
    "reader.episodeList": "에피소드 목록",
    "reader.previous": "이전",
    "reader.next": "다음",
    "reader.noContent": "에피소드 내용이 아직 수집되지 않았습니다.",
    "reader.glossaryHeading": "용어집",
    "reader.glossaryEmpty": "확정된 용어집 항목이 아직 없습니다.",
    "reader.glossaryShow": "용어집 보이기",
    "reader.glossaryHide": "용어집 숨기기",
    "reader.styleGuideHeading": "스타일 가이드",
    "reader.episodeOf": "{number} / {total}",
    "reader.endOfNovel": "완결 ✓",

    // Translation toggle
    "translation.translate": "번역",
    "translation.translating": "번역 중...",
    "translation.bilingual": "한국어와 일본어 문단을 나란히 표시",
    "translation.failedRetry": "실패 — 재시도",
    "translation.rateLimited": "요청 제한 — 다른 모델로 시도",
    "translation.retranslate": "재번역",
    "translation.selectModel": "번역 선택",
    "translation.model": "모델",
    "translation.currentModel": "현재 모델",
    "translation.openSettings": "모델 설정 열기",
    "translation.noSavedTranslations": "이 에피소드에는 아직 저장된 번역이 없습니다.",
    "translation.inventoryTitle": "번역 보관함",
    "translation.inventorySubtitle": "새 프롬프트나 모델로 다시 번역하기 전에 저장된 번역을 정리할 수 있습니다.",
    "translation.availableCount": "사용 가능",
    "translation.discardAll": "전체 삭제",
    "translation.discardModel": "모델 삭제",
    "translation.discardSingle": "번역 삭제",
    "translation.discardShort": "삭제",
    "translation.discarding": "삭제 중...",
    "translation.inventoryEmpty": "이 작품에는 아직 저장된 번역이 없습니다.",
    "translation.confirmDiscardSingle": "이 저장된 번역을 삭제할까요?",
    "translation.confirmDiscardAll": "이 작품의 저장된 한국어 번역을 모두 삭제할까요?",
    "translation.confirmDiscardModel": "모델 {model}의 저장된 번역을 삭제할까요?",
    "translation.totalCost": "총 비용",
    "translation.costLabel": "비용",
    "translation.costValue": "${cost}",
    "translation.costNotAvailable": "비용 정보 없음",
    "translation.modelCount": "{count}개 모델",
    "translation.requestFailed": "번역 요청을 시작하지 못했습니다. 다시 시도해 주세요.",
    "translation.loadFailed": "해당 번역을 불러오지 못했습니다.",
    "translation.discardFailed": "저장된 번역을 삭제하지 못했습니다.",
    "translation.discardSuccess": "저장된 번역을 삭제했습니다.",
    "translation.etaLabel": "예상 남은 시간",
    "translation.confidenceLow": "신뢰도 낮음",
    "translation.confidenceMedium": "신뢰도 보통",
    "translation.confidenceHigh": "신뢰도 높음",

    // Ranking
    "ranking.title": "랭킹",
    "ranking.subtitle": "나로우에서 인기 있는 소설을 찾아보세요.",
    "ranking.daily": "일간",
    "ranking.weekly": "주간",
    "ranking.monthly": "월간",
    "ranking.quarterly": "분기",
    "ranking.loading": "랭킹 불러오는 중...",
    "ranking.empty": "랭킹 데이터가 없습니다.",
    "ranking.eps": "화",
    "ranking.completed": "완결",
    "ranking.ongoing": "연재 중",
    "ranking.view": "보기",
    "ranking.register": "등록",
    "ranking.translatingTitles": "제목 번역 중...",
    "ranking.eyebrow": "발견",
    "ranking.headline": "Top of Syosetu",
    "ranking.headlineFlair": "Top of",
    "ranking.headlineMain": "Syosetu",
    "ranking.tagline": "나로우 종합 랭킹 — 매일 갱신, 한국어 제목 자동 번역.",

    // Settings
    "settings.title": "설정",
    "settings.language": "언어",
    "settings.readerSettings": "리더 설정",
    "settings.readerSettingsDesc": "글자 크기 · 줄 간격 · 콘텐츠 너비 — 리더 화면에 즉시 적용됩니다.",
    "settings.fontFamily": "글꼴",
    "settings.fontFamilyDesc": "본문 한자 · 가나 표시 폰트. 미리보기를 클릭해 선택합니다.",
    "settings.fontWeight": "굵기",
    "settings.fontSize": "글자",
    "settings.lineHeight": "줄간격",
    "settings.contentWidth": "너비",
    "settings.save": "저장",
    "settings.saving": "저장 중...",
    "settings.saved": "저장됨",
    "settings.toggleTheme": "테마 전환",
    "settings.themePaper": "페이퍼",
    "settings.themeSepia": "세피아",
    "settings.themeNight": "나이트",
    "settings.themeSystem": "시스템",
    "settings.themeSection": "테마",
    "settings.themeSectionDesc": "페이퍼, 세피아, 나이트, 시스템 모드를 전환합니다.",
    "settings.adultContent": "성인 소스",
    "settings.adultContentDesc": "랭킹·검색·라이브러리에서 R-18 소스(Nocturne, AlphaPolis 성인 작품)를 표시합니다.",
    "settings.adultContentOn": "표시",
    "settings.adultContentOff": "숨김",
    "source.syosetu": "Syosetu",
    "source.nocturne": "Nocturne",
    "source.kakuyomu": "Kakuyomu",
    "source.alphapolis": "AlphaPolis",
    "ranking.period.daily": "일간",
    "ranking.period.weekly": "주간",
    "ranking.period.monthly": "월간",
    "ranking.period.quarterly": "분기",
    "ranking.period.yearly": "연간",
    "ranking.period.entire": "전체기간",
    "ranking.period.hot": "인기",
    "ranking.scope.sfw": "전체이용가",
    "ranking.scope.all": "모든 소스",
    "ranking.sectionTimeout": "응답 지연 — 재시도?",
    "ranking.sectionError": "소스 사용 불가",
    "settings.translationConfig": "번역 모델과 프롬프트를 설정합니다.",
    "settings.translationModel": "번역 모델",
    "settings.translationModelDesc": "JA→KO 번역에 사용할 OpenRouter 모델을 선택하세요. 아래 워크로드별 설정이 이 기본값을 덮어씁니다.",
    "settings.currentModel": "현재 모델",
    "settings.workloadModels": "워크로드별 모델",
    "settings.workloadModelsDesc": "워크로드별로 모델을 다르게 지정합니다. 예: 본문 번역은 Flash, 용어집 부트스트랩은 Pro. 비워두면 환경 기본값 또는 위 기본 모델을 상속합니다.",
    "settings.workloadTranslate": "번역 · 본문",
    "settings.workloadTranslateHint": "에피소드 본문 번역. reasoning OFF, 출력 토큰 비용 중심.",
    "settings.workloadTitle": "제목 배치",
    "settings.workloadTitleHint": "랭킹/에피소드 제목 배치 번역. 짧은 출력.",
    "settings.workloadSummary": "세션 요약",
    "settings.workloadSummaryHint": "에피소드 간 컨텍스트 요약.",
    "settings.workloadExtraction": "용어집 추출",
    "settings.workloadExtractionHint": "에피소드별 신규 용어 JSON 추출.",
    "settings.workloadCompare": "재번역/비교",
    "settings.workloadCompareHint": "품질 민감한 단일 재번역 작업.",
    "settings.workloadBootstrap": "용어집 부트스트랩",
    "settings.workloadBootstrapHint": "번역 전 JA 형태소 마이닝 기반 용어집 생성.",
    "settings.workloadInherit": "상속",
    "settings.workloadInheritPlaceholder": "기본값 사용 (비워둠)",
    "settings.workloadReset": "상속으로 되돌리기",
    "settings.searchModels": "모델 검색...",
    "settings.loadingModels": "사용 가능한 모델을 불러오는 중...",
    "settings.noModelsFound": "모델을 찾을 수 없습니다.",
    "settings.refineSearch": "처음 50개 결과를 표시 중입니다. 검색어를 더 구체적으로 입력하세요.",
    "settings.globalPrompt": "전체 번역 프롬프트",
    "settings.globalPromptDesc": "모든 번역에 적용되는 추가 지시사항입니다.",
    "settings.globalPromptPlaceholder": "추가 번역 가이드라인을 입력하세요 (예: 존댓말 처리, 문체 선호 등)...",
    "settings.useDefault": "기본값 사용",
    "settings.eyebrow": "환경 설정",
    "settings.heading": "설정",
    "settings.headingFlair": "Settings",
    "settings.sectionAccount": "계정 · 프로필",
    "settings.sectionReading": "읽기 환경",
    "settings.sectionTranslation": "번역",
    "settings.sectionData": "데이터 · 동기화",
    "settings.profilesHint": "로컬 리더 프로필을 만들고 설정·읽기 진행 상태를 프로필별로 전환하세요.",
    "settings.profilesAction": "프로필 열기 →",
    "settings.themeSepiaBg": "#f1e2c7",

    // Profiles
    "profile.loading": "프로필 불러오는 중...",
    "profile.title": "프로필",
    "profile.subtitle": "로컬 리더 프로필을 만들고, 설정·읽기 진행 상태를 프로필별로 전환하세요.",
    "profile.create": "프로필 만들기",
    "profile.creating": "생성 중...",
    "profile.namePlaceholder": "프로필 이름",
    "profile.importGuestData": "현재 게스트 데이터를 이 프로필로 가져오기",
    "profile.listTitle": "사용 가능한 프로필",
    "profile.select": "선택",
    "profile.switching": "전환 중...",
    "profile.current": "현재 사용 중",
    "profile.empty": "아직 프로필이 없습니다.",
    "profile.guest": "게스트 프로필",
    "profile.useGuest": "게스트 사용",
    "profile.manage": "관리",
    "profile.active": "활성",
    "profile.loadFailed": "프로필을 불러오지 못했습니다. 새로고침 후 다시 시도해 주세요.",
    "profile.createFailed": "프로필을 만들지 못했습니다. 이름을 확인하고 다시 시도해 주세요.",
    "profile.selectFailed": "프로필 전환에 실패했습니다. 다시 시도해 주세요.",
    "profile.useGuestFailed": "게스트 모드로 전환하지 못했습니다. 다시 시도해 주세요.",
    "profile.createSuccess": "프로필을 만들고 바로 활성화했습니다.",
    "profile.switchSuccess": "프로필을 전환했습니다.",
    "profile.guestSuccess": "게스트 프로필이 활성화되었습니다.",

    // Glossary
    "glossary.title": "글로서리 & 가이드라인",
    "glossary.subtitle": "번역된 에피소드에서 용어집 항목 & 스타일 가이드 생성",
    "glossary.placeholder": "문체 가이드라인 (어조, 격식체/비격식체, 화법 패턴). 용어 매핑은 위의 항목 테이블에서 관리됩니다.",
    "glossary.generate": "생성",
    "glossary.generating": "생성 중...",
    "glossary.generatedInfo": "{episodes}화 기반으로 {date}에 생성됨",
    "glossary.generateAction": "용어집 & 스타일 가이드 생성",
    "glossary.generateActionBusy": "생성 중...",
    "glossary.model": "모델",
    "glossary.estimatedCost": "예상 비용",
    "glossary.noEpisodes": "번역된 에피소드 없음",
    "glossary.episodes": "{count}화",
    "glossary.entries": "용어집 항목",
    "glossary.styleGuide": "스타일 가이드",
    "glossary.addEntry": "추가",
    "glossary.termJa": "일본어",
    "glossary.termKo": "한국어",
    "glossary.reading": "읽기",
    "glossary.notes": "비고",
    "glossary.category": "분류",
    "glossary.statusConfirmed": "확정",
    "glossary.statusSuggested": "제안",
    "glossary.statusRejected": "거부",
    "glossary.confirm": "확정",
    "glossary.reject": "거부",
    "glossary.delete": "삭제",
    "glossary.all": "전체",
    "glossary.character": "캐릭터",
    "glossary.place": "장소",
    "glossary.term": "용어",
    "glossary.skill": "스킬",
    "glossary.honorific": "호칭",
    "glossary.noEntries": "아직 용어집 항목이 없습니다.",
    "glossary.entryAdded": "항목 추가됨",
    "glossary.entryUpdated": "항목 수정됨",
    "glossary.entryDeleted": "항목 삭제됨",
    "glossary.entryAddFailed": "항목 추가 실패",
    "glossary.entryUpdateFailed": "항목 수정 실패",
    "glossary.entryDeleteFailed": "항목 삭제 실패",
    "glossary.saveFailed": "스타일 가이드 저장 실패",
    "glossary.generateFailed": "생성 실패",
    "glossary.networkError": "네트워크 오류",
    "glossary.generated": "스타일 가이드 생성 완료",
    "glossary.generatedWithEntries": "스타일 가이드 생성 완료, {count}개 항목 등록",
    "glossary.loading": "불러오는 중...",
    "glossary.save": "저장",
    "glossary.cancel": "취소",
    "glossary.importance": "중요도",

    // Auth
    "auth.loading": "불러오는 중...",
    "auth.title": "로그인",
    "auth.subtitle": "이메일로 로그인하여 개인 설정, 서재, 이어읽기 상태를 불러오세요.",
    "auth.email": "이메일",
    "auth.displayName": "표시 이름",
    "auth.displayNamePlaceholder": "선택 사항",
    "auth.signIn": "로그인",
    "auth.signingIn": "로그인 중...",
    "auth.signOut": "로그아웃",
    "auth.signedInAs": "로그인 사용자",
    "auth.invalidCredentials": "입력 내용을 확인하고 다시 시도해 주세요.",
    "auth.networkError": "네트워크 오류. 다시 시도해 주세요.",
    "auth.guestHint": "로그인하지 않고 사용할까요?",
    "auth.continueAsGuest": "게스트로 계속",

    // Locale names
    "locale.en": "English",
    "locale.ko": "한국어",
  },
} as const;

export type Locale = keyof typeof dictionaries;
export type TranslationKey = keyof (typeof dictionaries)["en"];
