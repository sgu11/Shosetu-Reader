    ...
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-8 px-6 py-10">
      ...
      <form onSubmit={handleSubmit} className="flex gap-3">
        <input
          ...
          className="flex-1 rounded-xl border hairline bg-surface px-4 py-3 text-sm placeholder:text-muted/60 focus:outline-none focus:ring-2 focus:ring-accent/40"
          disabled={loading}
        />
        <button
          type="submit"
          disabled={loading || !input.trim()}
          className="rounded-xl bg-accent px-6 py-3 text-sm font-semibold text-accent-contrast transition-opacity disabled:opacity-50"
        >
      ...
      {result && (
        <div className="surface-card space-y-4 rounded-2xl p-6">
          ...
          <h2 className="text-xl font-semibold">{result.novel.titleJa}</h2>
          ...
          <button
            ...
            className="rounded-xl bg-accent px-5 py-2.5 text-sm font-semibold text-accent-contrast transition-opacity hover:opacity-90"
          >
            View novel details
          </button>
        </div>
